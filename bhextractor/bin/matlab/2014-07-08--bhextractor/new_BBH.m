function new_BBH(run_name, noiseType, model, catalogue,wv,ra, dec, psi, trigtime, lowfreq, highfreq, seed, detno,numPCs,doPlot,doSky, typeofscaling, scaling, doDistance, doTimeshift)
...
%               = 1...10 when model ~= Ott
%               = 1...6 when model == Ott
%           doPlot -- flag to create plots to track progress of code
%           doSky --  flag to use ra, dc, psi, trigtime as additional
%           parameters
...
display(sprintf('new_BBH: %s %s %s %i', noise_curve, model, catalogue, wv));
...
clearvars -except run_name catalogue wv model seed lowfreq highfreq typeofscaling scaling ...
    detno numPCs doTimeshift doDistance doPlot noise_curve ra dec psi trigtime doSky BHEX_PREFIX
...
%Computes the timedelay (using the new methods) for each indicated detector
for cnt3=1:detno;
...
% The following segment is supposed to put the PCs in the Fourier domain and
% apply then apply a timeshift for each detector 
for cntdet=1:detno; 
...
%This applies the antenna pattern to the signal
 wave0=wave;
 for cntdet=1:detno;
...
% This is meant to apply scale factors when computing distance 
if(doDistance)
...
%                      deltaF, len, f,lowfreq_index, highfreq_index, activeT_range(j,k), activera(j),activedec(j), ...
%                      activepsi(j),activetmp(j),detector{i},@findy, V_ft(i).V_ft, ...
...
%              for i=1:detno
%                  lp = like_gauss_fspace_td(wave_ft(:,i), sigdet(:,i), ...
%                      deltaF, len, f,lowfreq_index, highfreq_index,activeT_range(j,k), ra,dec, ...
%                      psi,trigtime,detector{i}, @findy, V_ft(i).V_ft, ...
%                      activebeta(j,:), actived(j), numPCs);
%                  
%                  likeactive(j) = likeactive(j) + lp;% + sum(betaprior);
%                  
...
for j=1:numactive
    for i=1:detno
        lp = like_gauss_tspace(wave_ft(:,i), noisevec(:,i), sigdet(:,i), ...
            deltaF, len, f, lowfreq_index, highfreq_index,activeT(j), actived(j), activera(j),activedec(j),activepsi(j),...
            activetmp(j),detector{i},@findy, V_ft(i).V_ft, activebeta(j,:), ...
             actived(j),numPCs );
...
for j=1:numactive
    for i=1:detno
        lp = like_gauss_tspace(wave_ft(:,i), noisevec(:,i), sigdet(:,i), ...
            ...
            actived(j),ra,dec,psi,activetmp(j), ...
            detector{i}, @findy, V_ft(i).V_ft, activebeta(j,:), ...
            actived(j),numPCs);
        likeactive(j) = likeactive(j) + lp;% + sum(betaprior);
...
while j-2 <= numactive * infosafe * H(j-1)
    ...
    if(doPlot)
        ...
        y_fft = findy(V_ft(1).V_ft, activebeta_avg, activeD_avg, numPCs, activera_avg, ...
       ...
       % y_sig_fft = wave0hplus+wave0hcross;
        ...
        y_fft = findy(V_ft(1).V_ft, activebeta_avg, numPCs);%.*exp(-2*pi*1i*f.*trigtime);
        y = ifft([0; y_fft; 0; flipud(conj(y_fft))]);
        y_sig_fft = fft(wave0)*sample_deltaT;
        %y = ifft([0; y_fft; 0; flipud(conj(y_fft))]);
...
%             end
            ...
            for i=1:detno
                lp = like_gauss_fspace_td(wave_ft(:,i), sigdet(:,i), ...
                    deltaF, len, f,lowfreq_index, highfreq_index, Tnew,dnew, ranew,decnew,psinew,tmpnew,detector{i}, ...
                    @findy, V_ft(i).V_ft, betanew,dnew, numPCs);
            ...
            for i=1:detno
                lp = like_gauss_tspace(wave_ft(:,i), noisevec(:,i), sigdet(:,i), ...
                    deltaF, len, f, lowfreq_index, highfreq_index, Tnew,dnew, ra,dec,psi,tmpnew,detector{i},...
                    @findy, V_ft(i).V_ft, betanew, dnew,numPCs);
...
if(doSky)
posterior_params_savefile = ['new_BBH_output_' catalogue num2str(wv) '_model' model '_PCs' num2str(numPCs)...
...
save([resultsdir posterior_params_savefile],'catalogue','wv','model','betas','detno',...
    ...
    'postbetas','postT','postdis','postTmp','Z','likeactive','logw','postdis');
end

% save a simple txt file with a line of output
outFile = sprintf('%s/new_BBH_results.txt', resultsdir);
fid = fopen(outFile, 'a');
